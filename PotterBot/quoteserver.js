...
try {
   ...
   potter1 = fs.readFileSync("Harry Potter Book 1.txt", 'utf8').trim().split('\r\n');
} catch (err) {
   console.error(err);
   return;
}
let potterfinal = potter1.filter((value, index, array)=>{
   return value != '';
})

var newhope = []
try{
   //https://github.com/gastonstat/StarWars/blob/master/Text_files/EpisodeIV_dialogues.txt
   newhope = fs.readFileSync("NewHopeDialogue.txt", 'utf8').trim().split('\r\n');
} catch (err){
   console.error(err);
   return;
}

//Taken from https://codepen.io/dhuddleston/pen/yegXEJ
...
//End taken from https://codepen.io/dhuddleston/pen/yegXEJ

function generateNCSWQuote(){
   return newhope[Math.floor(Math.random() * newhope.length)];
}

function generateSWQuote() {
...
function generateHPQuote(){
   let quoteNum = Math.floor(Math.random() * potterfinal.length);
   return potterfinal[quoteNum];
}
//console.log(potterfinal);
